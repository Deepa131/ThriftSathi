...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    // The "Messages" badge was previously (incorrectly) showing the
    // notifications count. Compute the real unread-message total from
    // each conversation's unreadBuyer/unreadSeller counter instead.
    messagesAPI.getConversations()
      .then(({ data }) => {
        const total = (data.conversations || []).reduce((sum, c) => {
          const mine = String(user._id) === String(c.buyer?._id) ? c.unreadBuyer : c.unreadSeller;
          return sum + (mine || 0);
        }, 0);
        setUnread(total);
      })
